...
with open('./citations.tsv', 'at') as cites_file, open('./coauthors.tsv', 'at') as coaut_file:
    ...
    if len_output_file == 1:
        cites_writer.writerow(['twitter', 'name', 'h_index', 'h_2014', 'i10_index', 'i10_2014'] + list(range(1980, 2020)))
        coaut_writer.writerow(['twitter', 'name'                                              ] + list(range(   1,  100)))

    # Compute and write citation statistics form the scientists names and from their dblp / scholar webpages
    for i, (real_name, twitter_name, dblp_url) in enumerate(zip(data['real name'], data['screen name'], data['dblp url'])):
            ...
            cites_writer.writerow([twitter_name, real_name, h, h_2014, i10, i10_2014] + citations)
            coaut_writer.writerow([twitter_name, real_name                          ] + coauthors)
