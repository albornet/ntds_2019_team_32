...
if rebuild_from_raw:
    ...
    # Remove the scientists that have no publications, because we don't know if they really have
    twitter_data = twitter_data[list(scholar_data.loc[:,'2006':'2019'].sum(axis=1) > 0)].reset_index(drop=True)
    coauths_list = coauths_list[list(scholar_data.loc[:,'2006':'2019'].sum(axis=1) > 0)].reset_index(drop=True)
    scholar_data = scholar_data[list(scholar_data.loc[:,'2006':'2019'].sum(axis=1) > 0)].reset_index(drop=True)
    ...
    # Save the raw matrix
    numpy.save('A_coauthors_raw', A_coauthors)

# Load the raw matrix
else:
    A_coauthors = numpy.load('A_coauthors_raw.npy')
...
numpy.save('A_coauthors', A_coauthors)

# Build the degree distribution
D_coauthors = A_coauthors.sum(axis=0)
hist_axis   = numpy.ones(D_coauthors.shape[0])/D_coauthors.shape[0]
...
axes[1].hist(D_coauthors, weights=hist_axis)
plt.show()